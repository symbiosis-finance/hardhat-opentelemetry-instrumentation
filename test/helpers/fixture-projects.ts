...
import { createHardhatRuntimeEnvironment, importUserConfig, resolveHardhatConfigPath } from 'hardhat/hre'
 ...
 */
export async function createFixtureProjectHRE(fixtureProjectName: string): Promise<HardhatRuntimeEnvironment> {
  const fixtureProjectRoot = path.resolve(import.meta.dirname, `../fixture-projects/${fixtureProjectName}`)

  const configPath = await resolveHardhatConfigPath(path.join(fixtureProjectRoot, 'hardhat.config.ts'))
